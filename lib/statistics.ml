let evaluate_probability (values : int list) (conditions : (int -> bool) list) : float =
...
let cumulative_distribution_function target values =
  let[@tail_mod_cons] rec cumulative_distribution_function' target values accumulator =
    match target with
    | 0 ->
        accumulator
    | _ ->
        ...
        cumulative_distribution_function' (target - 1) values (accumulator +. probability)
  in
